...
function saveEventState(){localStorage.setItem('xodus-claims',JSON.stringify(claims));localStorage.setItem('xodus-actions',JSON.stringify(actionStack));localStorage.setItem('xodus-event-log',JSON.stringify(eventLog))}
...
function renderHistory(){const items=eventLog.slice(-10).reverse();$('eventHistory').innerHTML=items.length?items.map(x=>`<div class="history-item"><b>${x.type}</b> · ${x.label}</div>`).join(''):'<div class="history-item">No event actions yet.</div>';$('undoButton').disabled=!actionStack.length;$('lockButton').textContent=setupLocked?'Unlock setup':'Lock setup';$('teamCount').disabled=setupLocked}
...
$('teamCount').value=teamCount;$('teamCount').onchange=e=>{teamCount=Number(e.target.value);viewTeam=Math.min(viewTeam,teamCount-1);claims=Object.fromEntries(Object.entries(claims).filter(([,c])=>c.team<teamCount));localStorage.setItem('xodus-team-count',teamCount);localStorage.setItem('xodus-claims',JSON.stringify(claims));tiles=buildTiles();selected=null;$('emptyState').hidden=false;$('tileDetails').hidden=true;render()};
...
$('claimTeam').onchange=()=>{};
$('modeButton').onclick=()=>{organizer=!organizer;$('organizerTools').hidden=!organizer;$('adminPanel').hidden=!organizer;$('modeButton').classList.toggle('active',organizer);$('modeButton').textContent=organizer?'Organizer mode: On':'Organizer mode';if(organizer)renderHistory();toast(organizer?'Organizer controls unlocked':'Clan view restored')};
...
$('lockButton').onclick=()=>{setupLocked=!setupLocked;localStorage.setItem('xodus-setup-locked',setupLocked);renderHistory();toast(setupLocked?'Team count and layout locked':'Setup unlocked')};
